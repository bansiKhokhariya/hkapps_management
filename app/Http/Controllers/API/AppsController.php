...
class AppsController extends Controller
{
    public function index(){
        ...
        if (!$companyUser) {
            $app = AllApps::where('status','live')->filter()->latest()->paginate(50);
        } else {
            $app = AllApps::where('status','live')->where('app_accountName', $companyMaster->company)->filter()->latest()->paginate(50);
        }
    ...
    public function fetchAppData($package_name)
    {
        ...
        if ($res->status() == 200) {
            ...
            $get_app = AllApps::where('app_packageName', $package_name)->first();
            if (!($get_app)) {
                $app = new AllApps();
                $app->app_name = $app_response->title;
                $app->app_packageName = $package_name;
                $app->app_logo = $app_response->icon;
    ...
    public function getWebCreonPackage(){
        ...
        $getWebCreonPackage = AllApps::pluck('app_packageName');
