...
class AllApps extends Model
{
    ...
    public function appDetails(){
        ...
        $app_details = AppDetails::where('allApps_id', $this->id)->first();
        return $app_details;
    ...
    public function viewResponse( $package_name , $api_key ) {


        // ***************** view app response json ******************** //

        $all_apps = AllApps::where('app_packageName', $package_name)->where('app_apikey', $api_key)->first();

        if ($all_apps) {
            $get_app = AllApps::where('app_packageName', $package_name)->where('app_apikey', $api_key)->select('app_name', 'app_packageName', 'app_logo', 'app_privacyPolicyLink', 'app_updateAppDialogStatus', 'app_versionCode', 'app_redirectOtherAppStatus', 'app_newPackageName', 'app_adPlatformSequence', 'app_adShowStatus', 'app_AppOpenAdStatus', 'app_howShowAd', 'app_alternateAdShow', 'app_mainClickCntSwAd', 'app_innerClickCntSwAd', 'app_apikey', 'app_note', 'app_accountLink', 'app_testAdStatus')->first();
            ...
            $app_data = AllApps::where('app_packageName', $package_name)->where('app_apikey', $api_key)->first();
