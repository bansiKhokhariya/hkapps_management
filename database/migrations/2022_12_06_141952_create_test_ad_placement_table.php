...
class CreateTestAdPlacementTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('test_ad_placement', function (Blueprint $table) {
            $table->id();
...
//            $table->foreign('allApps_id')->references('id')->on('test_all_apps')->onDelete('cascade')->onUpdate('cascade');
//            $table->foreign('platform_id')->references('id')->on('platform')->onDelete('cascade')->onUpdate('cascade');
            $table->softDeletes();
            $table->timestamps();
        });
        Schema::table('test_ad_placement', function($table)
        {
            $table->foreign('allApps_id')->references('id')->on('test_all_apps');
    ...
    {
        Schema::dropIfExists('test_ad_placement');
    }
}
